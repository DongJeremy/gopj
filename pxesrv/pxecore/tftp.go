...
func (s *Server) tftpReadHandler(filename string, rf io.ReaderFrom) error {
	file, err := os.Open(s.Config.TFTP.Path + "/" + filename)
	if err != nil {
		log.Errorf("%v", err)
		return err
	}
...
func (s *Server) tftWriteHandler(filename string, wt io.WriterTo) error {
	file, err := os.OpenFile(s.Config.TFTP.Path+"/"+filename, os.O_WRONLY|os.O_CREATE|os.O_EXCL, 0644)
	if err != nil {
		log.Errorf("%v", err)
		return err
	}
	...
	log.Printf("TFTP: tftp_files recieved and stored file to %s", s.Config.TFTP.Path+"/"+filename)
	return nil
}
...
func (s *Server) serveTFTP(l *net.UDPConn) error {
	...
	log.Printf("starting tftp server and listening on port %s handle on path: %s", s.Config.TFTP.Port, s.Config.TFTP.Path)
